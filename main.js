...
button.addEventListener("click", ()=>{
  ...
  function getLocation(ip) {
    fetch(`https://ipinfo.io/${ip}/geo?token=6370439e94bf29`) 
      ...
      .then((data) => {
        ...
        let yourdatetime = new Date().toLocaleString("en-US", { timeZone: data.timezone });

        timeDiv.innerHTML = `
        <div><h4>Time Zone: ${data.timezone}</h4></div>
        <div><h4>Date And Time: ${yourdatetime} </h4></div>
  ...
  function getPostOffices(zip) {
    ...
    fetch(`https://api.postalpincode.in/pincode/${zip}`)
      .then((response) => response.json())
      .then((data) => {
        document.querySelector(".timeZone>:nth-child(4)").innerHTML = `<strong>Message:</strong> ${data[0].Message}`;
